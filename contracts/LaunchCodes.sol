...
contract LaunchCodes {
    ...
    event DoorStatusChanged(bool isOpen);
    ...
    modifier approvedRequest() {
        require(
            addressToRequest[msg.sender].isApprovedByFirstGuard &&
                addressToRequest[msg.sender].isApprovedBySecondGuard,
            "Request is not approved by both guards"
        );
        _;
    }
    ...
    function makeRequest(bool _isEntry) external {
        addressToRequest[msg.sender] = request(
            msg.sender,
            _isEntry,
            false,
            false
    ...
    function approveEntry(address _requester) external onlyGuard {
        require(
            addressToRequest[_requester].isEntry,
            "You can only approve entry requests"
        );
        require(
            !checkPersonInFacility(_requester),
            "The requester is already in the building"
        );
    ...
    function approveExit(address _requester) external onlyGuard {
        require(
            !addressToRequest[_requester].isEntry,
            "You can only approve exit requests"
        );
        require(
            _requester != firstGuard && _requester != secondGuard,
            "Guards cant exit"
        );
        require(
            checkPersonInFacility(_requester),
            "The requester is not in the building"
        );

        if (msg.sender == firstGuard) {
            //Hívó alapján jegyezzük fel, hogy melyik őr fogadta
            addressToRequest[_requester].isApprovedByFirstGuard = true;
        } else {
            addressToRequest[_requester].isApprovedBySecondGuard = true;
        }
    ...
    function Enter() external approvedRequest {
        ...
        if (isShiftChangeInProgress) {
            require(
                msg.sender == actualShiftChangeRequest.newGuard,
                "You cant enter during a shift change"
            );
        }
    ...
    function Exit() external approvedRequest {
        ...
        if (isShiftChangeInProgress) {
            require(
                msg.sender != actualShiftChangeRequest.newGuard,
                "You cant exit without shift change"
            );
        }
     ...
     * Eseményt is dob az ajtó állapotáról
     */
    function toggleDoor(bool _toOpen) internal {
        isDoorOpen = _toOpen;
        emit DoorStatusChanged(_toOpen);
    }
    ...
    function requestShiftChange() external {
        ...
        if (!lastChangeWasFirstGuard) {
            //Minden szolgálatváltáskor először az elsőt, majd a második őrt váltjuk le
            actualShiftChangeRequest = shiftChangeRequest(
                firstGuard,
                msg.sender,
                false
            );
    ...
    function approveShiftChange() external onlyGuard {
        require(
            msg.sender == actualShiftChangeRequest.oldGuard,
            "You cant approve shift change"
        );
        ...
        addressToRequest[actualShiftChangeRequest.newGuard] = request(
            actualShiftChangeRequest.newGuard,
            true,
            false,
            false
        );
    ...
    function completeShiftChange() external onlyGuard {
        require(
            actualShiftChangeRequest.isApproved,
            "Shift change is not approved"
        );
        require(
            checkPersonInFacility(actualShiftChangeRequest.newGuard),
            "The new guard is not in the building"
        );

        if (
            !lastChangeWasFirstGuard &&
            actualShiftChangeRequest.oldGuard == firstGuard
        ) {
            ...
            lastChangeWasFirstGuard &&
            actualShiftChangeRequest.oldGuard == secondGuard
        ) {
     ...
     */
    function checkPersonInFacility(
        address _address
